...
import {
    type Message,
    type FetchOrRestoreImageMessage as FetchOrRestoreImageMessage,
    type SerializedImage,
    type CompletedTranslationMessage,
    type StartedTranslationMessage,
    type FailedTranslationMessage,
    MessageType,
} from "../shared/types";
import {
    getContextMenuSourceTarget,
    getSourceById,
    getTargetElement,
    SourceTags,
} from "./sources";
...
const fetchOrRestoreImage: (
    ...args: FetchOrRestoreImageMessage["args"]
    ...
    return sources
        ...
        .map((c) => c.getImageInfo());
};

const onTranslationStarted: (
    ...args: StartedTranslationMessage["args"]
) => void = (imageId) => {
...
const onTranslationCompleted: (
    ...args: CompletedTranslationMessage["args"]
) => void = (sourceId, newUrl) => {
...
const onTranslationFailed: (
    ...args: FailedTranslationMessage["args"]
) => void = (imageId, reason) => {
    const source = getSourceById(imageId);
    ...
    console.error(
        `Failed to translate image with ${SourceTags.Id}="${imageId}": ${reason}`,
    );
};
...
const fetchBlobImage = async (url: string) => {
    const blob = await fetch(url).then(c => c.blob())
    const str = await blobToBase64Async(blob)
    return str
}
browser.runtime.onMessage.addListener((data, _, respond) => {
    ...
    switch (message.type) {
        ...
        case MessageType.InspectMessage:
            ...
            break;
        case MessageType.FetchBlobImage:
            {
                fetchBlobImage(...message.args).then(c => {
                    respond(makeResponse(c))
                }).catch((e) => {
                    respond(makeError(e))
                })
            }
