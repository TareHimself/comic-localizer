...
export class PictureSource implements ISource {
    ...
    constructor(source: HTMLPictureElement) {
        ...
        if (imageElement === null) {
            throw new Error(
                `Picture element with id ${this.id} does not have an image, what even is this website?`,
            );
        }
        this.sourceImage = imageElement;
        this.source.setAttribute(
            SourceTags.OriginalSrc,
            this.sourceImage.currentSrc ?? this.sourceImage.src,
        );

        this.sourceElements = Array.from(this.source.children)
            .filter((c) => !(c instanceof HTMLImageElement))
            .map((c) => {
                c.remove();
                return c;
            });
