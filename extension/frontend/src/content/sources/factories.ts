    ...
    if (
        ...
        return {
            context: {
                element: element,
                source:
                    existing.get(element) ??
                    new PictureSource(element.parentElement),
            },
            related: [],
        };
    }
...
const imageFactory: SourceFactory = (element, existing) => {
    if (element instanceof HTMLImageElement) {
        return {
            ...
            related: sortAndInterleave(
                Array.from(element.parentElement?.children ?? []).filter(
                    (c) => c instanceof HTMLImageElement,
                ) as HTMLImageElement[],
                element,
            ).map((e) => {
                return {
...
const welomaImageFactory: SourceFactory = (element, existing) => {
    if (
        ...
        window.location.origin.includes("https://weloma.")
    ) {
        return {
            context: {
                element,
...
const mangaFireImageFactory: SourceFactory = (element, existing) => {
    if (
        ...
        window.location.origin.includes("https://mangafire.") &&
        element.children[0]?.children[0] instanceof HTMLImageElement
    ) {
        ...
        return {
            ...
            related: sortAndInterleave(
                Array.from(element.parentElement?.children ?? []).filter(
                    (c) => {
                        const targetChild = c.children[0]?.children[0];
                        return targetChild instanceof HTMLImageElement && targetChild.hasAttribute("data-number") && (targetChild.parentElement?.classList.contains("loaded") ?? false)
                    },
                ) as HTMLImageElement[],
                element,
            ).map((e) => {
...
const comicWalkerFactory: SourceFactory = (element, existing) => {
    if (
        window.location.origin.startsWith("https://comic-walker.") &&
        element instanceof HTMLDivElement &&
        element.parentElement?.children[0] instanceof HTMLCanvasElement
    ) {
        return {
            context: {
                element,
                source:
                    existing.get(element) ??
                    new CanvasSource(element.parentElement?.children[0]),
            },
            related: sortAndInterleave(
                Array.from(
                    ...
                    (c) =>
                        c.children[0].children[0] instanceof HTMLCanvasElement,
                ) as HTMLDivElement[],
                element.parentElement.parentElement as HTMLDivElement,
            ).map((e) => {
                const contextElement = e.children[0]
                    .children[1] as HTMLDivElement;
                return {
                    element: contextElement,
                    source:
                        existing.get(contextElement) ??
                        new CanvasSource(
                            e.children[0].children[0] as HTMLCanvasElement,
                        ),
                };
            }),
