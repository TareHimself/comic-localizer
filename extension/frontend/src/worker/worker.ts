...
import {
    ...
    type FetchOrRestoreImageMessage,
    type InspectMessage,
...
const CONTEXT_MENU_TRANSLATE_OR_RESTORE_ID =
...
const sendTranslationCompletedMessage = async (
    tabId: number,
    imageId: string,
    newUrl: string,
) => {
...
const sendTranslationFailedMessage = async (
    tabId: number,
    imageId: string,
    reason: string,
) => {
        ...
        case CONTEXT_MENU_TRANSLATE_OR_RESTORE_ID:
            {
                ...
                const result: MessageResponse<SerializedImage[] | undefined> =
                    await browser.tabs.sendMessage(tabId, message);
                if (result.error) {
                ...
                await Promise.allSettled(
                    result.response.map((c) =>
                        sendTranslationStartedMessage(tabId, c.id),
                    ),
                );

                const promises = await TRANSLATOR.enqueue(tabId,result.response);
                promises.forEach((p, idx) => {
                    p.then(async (newImageUrl) => {
                        await sendTranslationCompletedMessage(
                            tabId,
                            result.response![idx].id,
                            newImageUrl,
                        );
                    }).catch((e) => {
        ...
        case CONTEXT_MENU_INSPECT:
            {
                ...
                const result: MessageResponse<string> =
