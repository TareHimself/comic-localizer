...
class OpenCvCleaner(Cleaner):
    ...
    def clean_frame(self, frame: np.ndarray, mask: np.ndarray):
        inpaint = cv2.inpaint(
            frame, mask, inpaintRadius=self.radius, flags=cv2.INPAINT_TELEA
        )
    ...
    async def clean(
        ...
        return await asyncio.gather(
            *[
                asyncio.to_thread(self.clean_frame, frame, mask)
    ...
    def get_arguments() -> list[PluginArgument]:
        return [
            IntPluginArgument("radius", "Radius", "Inpainting radius", 1),
            BooleanPluginArgument(
                "mask_after_inpaint", "Mask After Inpaint", "", False
            ),
        ]
