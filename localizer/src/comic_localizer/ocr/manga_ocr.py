...
import jaconv
import numpy as np
from comic_localizer.core.plugin import PytorchDevicePluginArgument
import torch
...
from comic_localizer.utils import cv2_to_pil, get_default_torch_device
import asyncio
...
class MangaOCR(OCR):
    ...
    def __init__(
        self,
        model_url: str = "kha-white/manga-ocr-base",
    ...
    def extract_with_model(self, batch: list[np.ndarray]):
        with torch.inference_mode():
            ...
            return [
                OcrResult(text=self.post_process(x), language=self.output_language)
                for x in y
            ]
    ...
    def get_arguments() -> list[PluginArgument]:
        return [PytorchDevicePluginArgument("device", "Device")]
