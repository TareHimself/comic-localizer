...
class OpenAiColorDetector(ColorDetector):
    ...
    def __init__(self, api_key: str, model=MODELS[0][1]) -> None:
        ...
        self.model = model
        self.instructions = """Auto-detect the color of the text, the outline size and outline color if it has one
IMPORTANT:
...
"""
    ...
    def do_color_detection(self, batch: list[np.ndarray]):
        ...
        if response.output_parsed is not None:
            return [
                ColorDetectionResult(
                    text_color=color_to_numpy(x.text_color),
                    outline_size=x.outline_size,
                    outline_color=(
                        color_to_numpy(x.outline_color) if x.outline_size > 0 else None
                    ),
                )
        ...
        return [
            ...
            SelectPluginArgument(
                ...
                options=[
                    ...
                    for a in OpenAiColorDetector.MODELS
                ],
